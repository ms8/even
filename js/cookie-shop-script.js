...
function setCookie(Type,No,Name,Price,Pic,Quantity){
    var n = parseInt(Quantity);
    if(!isNaN(n)){
        var valueStr = "|No="+No+"|Name="+Name+"|Price="+Price+"|pic="+Pic+"|Quantity="+Quantity
            +"|total="+parseFloat(Price)*n+"|type="+Type;
        addCookie(Type,No,valueStr);
        return true;
    }
    return false;
}

function addCookie(Type,No,NameStr){
    NameStr = "shop_xinya_" + Type +"_"+ No + "=" + NameStr;
    document.cookie = NameStr + ";" + "path=/";
}

function checkProduct(Type,No){
    var item = getCookie("shop_xinya_"+Type+"_"+No);
    if(item == ""){
        return false;
    }else{
        return true;
...
function addProduct(Type,No,Name,Price,Pic,Quantity){
    var item = getCookie("shop_xinya_"+Type+"_"+No);
    if(item == ""){
        return setCookie(Type,No,Name,Price,Pic,Quantity);
    }
//    if(item != ""){ //已经存在这个商品
//        var quote = "|Quantity=";
...
function parseProduct(product){
    ...
    var type = search(product,"|type=");
    if(no == "" || name == "" || price == "" || quantity == ""||total == ""||pic=="" ||type==""){
        return null
    }
    pObj.No = no;
    pObj.Name = name;
...
function getAllProduct(){
    if (document.cookie.length>0){
        ...
        var c_start = tmp.indexOf("shop_xinya_");
        while (c_start!=-1){
            ...
            if (c_end == tmp.length)
                break;
            c_start = tmp.indexOf("shop_xinya_",c_end);
        }
        return products;
    }
    return null;
...
function delProduct(Type,productNo){
    document.cookie = "shop_xinya_"+Type+"_"+productNo+"=dispear;Expires=Saturday,15 Sep 1990 23:59:59 GMT;path=/";
}
