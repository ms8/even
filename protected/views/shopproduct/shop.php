...
//    };
    ...
    function loadData() {
        ...
        $.ajax({
            ...
            dataType:'json',
            async:false,
            url: '?r=shopproduct/listIndex',
            //data:{'page':page},
            data:{'page':page,'type':active_type,'fid':fid},
            success: onLoadData
        });
...
//        });
    ...
    function onLoadData(data) {
        ...
        for(; i<length; i++) {
            ...
            html += '<a class="indexpic" href="?r=product/view&id='+image.pid+'&fid='+image.fid+'">'
            +'<img src="'+image.productpic+'" alt="Placeholder" class="thumbnail"></a>'
            +'<div style="margin:12px 0 35px 0;width: 180px;"><a href="?r=product/view&id='
                +image.pid+'&fid='+image.fid+'">'
            +image.pname+'</a>&nbsp;&nbsp;&nbsp;&nbsp;￥：'
            +image.productprice+'<img src="upload/cart.png" onclick="addItem('
                +image.pid+',\''+image.pname+'\','+image.productprice+',\''+image.productpic+'\')" alt="购物车" style="float:right;margin-top:-5px;cursor:pointer;"></div></div>';
        }
